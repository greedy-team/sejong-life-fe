    ...
    const mockReviews = [
      {
        ...
        liked: true,
        content:
          '정말 맛있는 카페였어요! 크로와상이 특히 바삭하고 맛있었습니다. 인테리어도 예쁘고 직원분들도 친절하셨어요. 다음에도 또 올 예정입니다.정말 맛있는 카페였어요! 크로와상이 특히 바삭하고 맛있었습니다. 인테리어도 예쁘고 직원분들도 친절하셨어요. 다음에도 또 올 예정입니다.정말 맛있는 카페였어요! 크로와상이 특히 바삭하고 맛있었습니다. 인테리어도 예쁘고 직원분들도 친절하셨어요. 다음에도 또 올 예정입니다.정말 맛있는 카페였어요! 크로와상이 특히 바삭하고 맛있었습니다. 인테리어도 예쁘고 직원분들도 친절하셨어요. 다음에도 또 올 예정입니다.정말 맛있는 카페였어요! 크로와상이 특히 바삭하고 맛있었습니다. 인테리어도 예쁘고 직원분들도 친절하셨어요. 다음에도 또 올 예정입니다.',
        likeCount: 24,
...
export const toggleReviewLike = http.post(
  '/api/places/:placeId/reviews/:reviewId/likes', // 프론트랑 맞춤
  ({ params }) => {
    ...
    const review = mockReviews.find((r) => r.reviewId === Number(reviewId));

    if (!review) {
      return HttpResponse.json(
        {
          message: '존재하지 않는 리뷰 조회',
          data: null,
        },
        { status: 404 },
      );
    ...
    return HttpResponse.json(
      {
        message: '리뷰 좋아요 성공',
        data: { reviewId: review.reviewId, likeCount: review.likeCount },
      },
      { status: 200 },
    );
  },
